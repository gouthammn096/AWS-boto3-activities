...
def lambda_handler(event, context):
    ...
    for instance in instances:
        print(
            "Instance name:"
            + [res["Value"] for res in instance["Tags"] if res["Key"] == "Name"][0]
        )

        # Default retention for 7 days if the tag is not specified
        try:
            retention_days = [
                int(t.get("Value")) for t in instance["Tags"] if t["Key"] == "Retention"
            ][0]
            ...
            try:
                ...
                print(
                    ...
                    % (AMIid["ImageId"], instance["InstanceId"], retention_days,)
                )
            ...
            # If the instance is not in running state
                ...
                print(
                    "Unexpected error, instance "
                    + [
                        res["Value"] for res in instance["Tags"] if res["Key"] == "Name"
                    ][0]
                    + "-"
                    + '"'
                    + instance["InstanceId"]
                    + '"'
                    + " might be in the state other then 'running'. So, AMI creation skipped."
                )
            except ClientError as e:
                if e.response["Error"]["Code"] == "InvalidAMIName.Duplicate":
                    print("AMI already exists")
                else:
